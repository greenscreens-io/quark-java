...
public class QuarkHandler {
    ...
    private void prepareHTTP() throws IOException {
        ...
        final JsonNode node = QuarkDecoder.decode(body, quarkKey);
        request = QuarkJson.convert(WebRequest.class, node);
        QuarkDecoder.decode(request, quarkKey);
    }
    ...
    protected boolean send() {
        ...
        try {
            if (Objects.isNull(wsSession)) {
                sendHTTP();
            } else if (wsSession.isOpen() && wsSession.isValidHttpSession()) {
                sendWS();
            }           
    ...
    private boolean isEncrypted() {
        return Objects.nonNull(quarkKey);
    }
     ...
     * @throws IOException
     */
    private IQuarkKey getCrypto() {
        if (Objects.nonNull(quarkKey)) return quarkKey;
        return isWebSocket() ? getCryptoWs() : getCryptoWeb();
    }
     ...
     * @throws IOException
     */
    private IQuarkKey getCryptoWeb() {
